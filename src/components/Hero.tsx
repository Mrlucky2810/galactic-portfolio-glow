...
const Hero = () => {
  ...
  const floatingCodeSnippets = ['public static void main(String[] args)', 'Widget build(BuildContext context)', 'const supabase = createClient()', 'npm run build && vercel deploy', '{ "flutter": "^3.24.0" }', 'FirebaseAuth.instance.signIn()', 'useEffect(() => {}, [])', 'mvn clean install', 'class SpringBootApp { }', 'flutter create my_app', 'git commit -m "feature"', 'docker build -t app .'];
  const stats = [{
    icon: Code,
    label: "Projects",
    value: "15+"
  ...
  return <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-10">
        {floatingCodeSnippets.map((snippet, index) => <motion.div key={index} initial={{
        opacity: 0,
        scale: 0
      }} animate={{
        opacity: [0, 0.7, 0],
      ...
      <div className="container mx-auto px-4 sm:px-6 z-20 relative">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="text-center">
          ...
          {/* Main Heading */}
          <motion.h1 variants={itemVariants} className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-space font-bold mb-4 sm:mb-6">
          ...
          <motion.div variants={itemVariants} className="mb-8 sm:mb-12">
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4 max-w-2xl mx-auto">
              {["Java", "Spring Boot", "Flutter", "Firebase", "React", "Supabase", "Vercel", "TypeScript"].map((tech, index) => <span key={index} className="px-3 py-1 text-xs sm:text-sm bg-gradient-to-r from-neon-cyan/20 to-neon-purple/20 border border-neon-cyan/30 rounded-full text-neon-cyan font-mono">
                  {tech}
                </span>)}
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="flex justify-center space-x-6 sm:space-x-8 mb-12 sm:mb-16">
            ...
            icon: Mail,
            href: 'mailto:shriyan.jaiswal@gmail.com',
            label: 'Email'
          }].map((social, index) => <motion.a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" whileHover={{
            scale: 1.2,
            y: -5
